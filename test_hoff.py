...
def test_computer_tables(page: Page):
    ...
    with allure.step("Add a table to the cart and place the order"):
        ...
        computer_table_tab.get_by_role(
            "button", name="Оформить заказ", exact=True
        ).click()
        expect(
...
@allure.description("This test attempts to add couple items to the basket and then emptify it.")
@allure.tag(
    "Essentials",
)
@allure.severity(allure.severity_level.MINOR)
...
def test_basket_emptify(page: Page):
    ...
    with allure.step("Check kitchen set reviews tab and add the set to the basket"):
        with page.context.expect_page() as second_tab:
            page.get_by_role(
                "link", name=re.compile(" кухонный гарнитур ")
            ).first.click()
    ...
    with allure.step("Open tabletop tab and add third item to the basket"):
    ...
    with allure.step("Open the basket and delete all items"):
